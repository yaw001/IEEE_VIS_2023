...
#group_1
boundary = sqrt(0.08)*2
sample_list = list()
sample_list_1 = list()
N = 100
for(i in size){
  for(j in 1:N){
    x_coord = mvrnorm(i, c(0,0), diag(2)*0.08, tol=1e-6, empirical = TRUE)[,1]
    y_coord = mvrnorm(i, c(0,0), diag(2)*0.08, tol=1e-6, empirical = TRUE)[,2]
...
setwd("/Users/young/Desktop/UCSD/Research/IEEE_VIS_2023/Experiment_code/Experiment_2_Precision/dist")
